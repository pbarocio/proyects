...
def obtener_historial_completo_df():
    """Consulta modularmente las 5 tablas de responsivas y las concatena en Python sin errores de colación."""
    ...
    try:
        ...
        if dfs:
            ...
            df_total["fecha_entrega_dt"] = pd.to_datetime(df_total["fecha_entrega"], errors="coerce")
            df_total["fecha_entrega"] = df_total["fecha_entrega_dt"].dt.strftime("%Y-%m-%d").fillna("Sin fecha")
            return df_total.sort_values(by="fecha_entrega_dt", ascending=False).reset_index(drop=True)
        return pd.DataFrame()

    except Exception as e:
        conn.close()
        st.error(f"⚠️ Error al consultar historial de responsivas: {e}")
        return pd.DataFrame()
...
def render_consulta():
    ...
    # --------------------------------------------------------------------------
    # FILTROS SUPERIORES: BUSCADOR LIBRE + SELECTORES
    # --------------------------------------------------------------------------
    c_busq, c1, c2, c3 = st.columns([1.5, 1.2, 1, 1])
    with c_busq:
        txt_busq_libre = st.text_input("🔍 Búsqueda Libre:", placeholder="Ej. IMEI, Serie, Folio, Número, Hostname...", key="filtro_hist_libre")
    with c1:
        colaboradores = ["Todos"] + sorted(list(df_historial["colaborador"].dropna().unique()))
        emp_sel = st.selectbox("Filtrar por Colaborador:", colaboradores, key="filtro_hist_colab")
    with c2:
        tipos_disp = ["Todos"] + sorted(list(df_historial["tipo"].dropna().unique()))
        tipo_sel = st.selectbox("Tipo de Equipo:", tipos_disp, key="filtro_hist_tipo")
    with c3:
        estatus_opts = ["Todos"] + sorted(list(df_historial["Estatus Documento"].dropna().unique()))
        estatus_sel = st.selectbox("Estatus:", estatus_opts, key="filtro_hist_estatus")
    ...
    if txt_busq_libre.strip():
        term = txt_busq_libre.strip().lower()
        cols_a_buscar = ["folio", "colaborador", "tipo", "id_equipo", "numero_linea", "equipo_descripcion", "numero_serie", "hostname", "marca", "modelo"]
        mascara = pd.Series(False, index=df_filtrado.index)
        for col in cols_a_buscar:
            if col in df_filtrado.columns:
                mascara |= df_filtrado[col].astype(str).str.lower().str.contains(term, na=False)
    ...
    st.caption(f"Mostrando **{len(df_filtrado)}** de **{len(df_historial)}** documentos responsivos.")
    ...
    if not df_filtrado.empty:
        opciones_descarga = []
        for idx_row, r in df_filtrado.iterrows():
            id_txt = f" (ID/IMEI: {r['id_equipo']})" if r['id_equipo'] else ""
            opciones_descarga.append(f"{idx_row} | {r['folio']} | {r['colaborador']} - {r['tipo']} {r['equipo_descripcion']}{id_txt} [{r['Estatus Documento']}]")

        key_dinamica = f"sel_resp_{emp_sel}_{tipo_sel}_{estatus_sel}_{txt_busq_libre.strip()}"

        sel_doc_txt = st.selectbox(
        ...
        if sel_doc_txt:
            idx_real = int(sel_doc_txt.split(" | ")[0])
            row_sel = df_filtrado.iloc[idx_real]
            ...
            if buffer_docx:
                st.download_button(
                    ...
                    type="primary",
                    key=f"btn_dl_{row_sel['folio']}_{idx_real}"
                )
            else:
                st.error("No se pudo generar el archivo. Verifica que las plantillas .docx existan en la ruta correspondiente.")
    else:
        st.info("No hay documentos responsivos que coincidan con los filtros o búsqueda ingresada.")
