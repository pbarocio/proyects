...
def generar_responsivas_word(df_responsivas, plantilla_path, output_dir):
    ...
    for idx, row in df_responsivas.iterrows():
        ...
        contexto = {
            ...
            'modelo': str(row['modelo']) if pd.notna(row['numero_serie']) else '',
        ...
        nombre_archivo = f"Responsiva Monitor {row['empleado']} ({row['marca']} {row['modelo']} {row['numero_serie']}).docx"
...
if __name__ == "__main__":
	...
	FROM responsivas_monitores rmon
	LEFT JOIN empleados emp ON rmon.codigo_empleado = emp.codigo
